...
public class PedidoServiceImpl implements IPedidoService {
	...
	public ArrayList<Pedido> filtrarPorVariosCriterios(FiltroPedidoDTO filtroPedidoDto) {
		...
		boolean temStatus = filtroPedidoDto.getCancelado() != 0 || filtroPedidoDto.getEntregue() != 0
				|| filtroPedidoDto.getPago() != 0;
		...
		if (!temNome && !temData && !temStatus) {
			...
			return dao.findAllByDataPedidoBetweenOrderByDataPedidoDesc(filtroPedidoDto.getDataInicio(),
					filtroPedidoDto.getDataFim());
			...
			return dao.findAllByDataPedidoBetweenAndStatusInOrderByDataPedidoDesc(filtroPedidoDto.getDataInicio(),
					filtroPedidoDto.getDataFim(), this.getStatus(filtroPedidoDto));
			...
			ArrayList<Cliente> clientes = clienteDao.findByNomeContaining(filtroPedidoDto.getNome());
			return dao.findAllByDataPedidoBetweenAndClienteInOrderByDataPedidoDesc(filtroPedidoDto.getDataInicio(),
					filtroPedidoDto.getDataFim(), clientes);
			...
			ArrayList<Cliente> clientes = clienteDao.findByNomeContaining(filtroPedidoDto.getNome());
			return dao.findAllByDataPedidoBetweenAndClienteInAndStatusInOrderByDataPedidoDesc(
					filtroPedidoDto.getDataInicio(), filtroPedidoDto.getDataFim(), clientes,
					this.getStatus(filtroPedidoDto));
		}
	...
	private Collection<Integer> getStatus(FiltroPedidoDTO filtroPedidoDTO) {
		...
		if (filtroPedidoDTO.getPago() != 0)
			status.add(filtroPedidoDTO.getPago());
		if (filtroPedidoDTO.getCancelado() != 0)
			status.add(filtroPedidoDTO.getCancelado());
		if (filtroPedidoDTO.getEntregue() != 0)
			status.add(filtroPedidoDTO.getEntregue());
		return status;
